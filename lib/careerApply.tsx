...
export default function CareerApply() {
    ...
    const [RequirementsCareer, setRequirementsCareer] = useState<RequirementCareer[]>([]);
    useEffect(() => {
    ...
    const handleChangeCareerMessage = (e: React.ChangeEvent<HTMLInputElement>) => {
    ...
    const handleCareerMessage = async () => {
        if (!formDataCareerMessage.name || !formDataCareerMessage.email || 
            !formDataCareerMessage.from || !formDataCareerMessage.phone ||
  ...
  return (
    <div className=' lg:w-[1600px] lg:h-180 lg:mt-30 lg:flex lg:flex-row lg:justify-center lg:items-start lg:relative w-[300px] h-240 mt-0 flex flex-col justify-center items-start relative'>
        ...
        {showPopup ?
        <>
            <div className='fixed z-7 w-80 lg:top-40 lg:left-70 top-40 p-5 border-1 rounded-lg border-purple-900 bg-white flex flex-col gap-3 justify-center items-center'>
                <h1 className='font-bold text-xl text-[#00a50b] w-full'>Apply Career</h1>
                <p className='text-[12px] text-green-900 w-full text-justify'>Chosen Career : <span className='font-bold'>{pickTitle}</span></p>
                <input 
                    type="text" className='w-full py-2 px-4 border border-green-900 bg-green-100 rounded-full text-[12px]'
                    placeholder='Your Name'
                    name="name"
                    value={formDataCareerMessage.name}
                    onChange={handleChangeCareerMessage}/>
                <input 
                    type="email" className='w-full py-2 px-4 border border-green-900 bg-green-50 rounded-full text-[12px]' 
                ...
                <input 
                    ...
                    onChange={handleChangeCareerMessage}/>
                <input 
                    type="text" className='w-full py-2 px-4 border border-green-900 bg-green-50 rounded-full text-[12px]' 
                    placeholder='Your Phone'
                    name="phone"
                    value={formDataCareerMessage.phone}
                    onChange={handleChangeCareerMessage}/>
                <button onClick={() => (handleCareerMessage())} className='text-[12px] font-bold text-white w-full border py-3 rounded-lg bg-[#007924] hover:bg-[#44975d]'>
                    {isLoading ? "Try Send..." : "Send"}
                </button>
                <button onClick={() => (setShowPopup(false))} className={`fixed z-6 lg:top-37 lg:right-45 lg:mr-0 -mr-40 top-36 w-8 h-8 rounded-full bg-[#AD48FF] flex justify-center items-center hover:bg-gradient-to-b hover:from-[#AD48FF] hover:to-[#6f09c3]`}>
                    <Image width={140} height={140} src="/close.svg" alt="" className="w-3"/>
                </button>
            </div>
            <div className='fixed z-6 w-115 lg:top-40 lg:right-50 top-40 p-5 border-1 rounded-lg border-purple-900 bg-white flex flex-col gap-3 justify-center items-center'>
                <div className='w-full relative flex flex-row items-center'>
                    <p className='text-[12px] font-bold text-green-900 w-13 text-justify'>Rate : </p>
                    <input 
                    type="number" className='appearance-none [&::-webkit-inner-spin-button]:appearance-none [&::-webkit-outer-spin-button]:appearance-none w-full py-2 px-4 pl-7 border border-green-900 bg-green-100 rounded-full text-[12px]' placeholder='0'
                    name="rate"
                    value={formDataCareerMessage.rate}
                    onChange={handleChangeCareerMessage}/>
                    <Image width={140} height={140} src="/dollar.svg" alt="MySQL" className=" lg:w-2 lg:absolute lg:top-3 lg:left-15 w-2 absolute top-28 left-58" />
                </div>
                <textarea 
                    name="message" 
                    id="message" className='w-full h-64 py-2 px-4 border border-green-900 bg-green-50 rounded-lg text-[12px]' 
        ...
        <>
            ...
            <div className='fixed z-9 lg:top-40 lg:left-140 top-40 p-5 border-1 rounded-lg border-purple-900 bg-white flex flex-col gap-3 justify-center items-center'>
                <Image width={140} height={140} src="/warning-red.svg" alt="" className="w-10"/>
                ...
                <button onClick={() => (setShowAuthYet(false))} className={`fixed z-6 lg:top-37 lg:right-133 lg:mr-0 -mr-40 top-36 w-8 h-8 rounded-full bg-[#AD48FF] flex justify-center items-center hover:bg-gradient-to-b hover:from-[#AD48FF] hover:to-[#6f09c3]`}>
                    <Image width={140} height={140} src="/close.svg" alt="" className="w-3"/>
                </button>
            </div>
        </>
