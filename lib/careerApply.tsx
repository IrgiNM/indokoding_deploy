        ...
        <>
            <div className='fixed z-7 lg:w-80 w-75 lg:top-40 lg:left-70 top-27 p-5 lg:border-1 rounded-lg border-purple-900 bg-white flex flex-col gap-3 justify-center items-center'>
                ...
                <input 
                    ...
                    onChange={handleChangeCareerMessage}/>
                <button onClick={() => (handleCareerMessage())} className='text-[12px] lg:flex justify-center items-center hidden font-bold text-white w-full border py-3 rounded-lg bg-[#007924] hover:bg-[#44975d]'>
                    {isLoading ? "Try Send..." : "Send"}
                </button>
                <button onClick={() => (setShowPopup(false))} className={`fixed z-6 lg:top-37 lg:right-45 lg:mr-0 -mr-40 top-24 right-45 w-8 h-8 rounded-full bg-[#AD48FF] flex justify-center items-center hover:bg-gradient-to-b hover:from-[#AD48FF] hover:to-[#6f09c3]`}>
                    <Image width={140} height={140} src="/close.svg" alt="" className="w-3"/>
                </button>
            </div>
            <div className='fixed lg:z-6 z-7 lg:w-115 w-75 lg:top-40 lg:right-50 top-94 p-5 lg:border-1 rounded-lg border-purple-900 bg-white flex flex-col gap-3 justify-center items-center'>
                <div className='w-full relative flex flex-row items-center'>
                    ...
                    onChange={handleChangeCareerMessage}/>
                    <Image width={140} height={140} src="/dollar.svg" alt="MySQL" className=" lg:w-2 lg:absolute lg:top-3 lg:left-15 w-2 absolute top-3 left-14" />
                </div>
                <textarea 
                    name="message" 
                    id="message" className='w-full lg:h-64 h-20 py-2 px-4 border border-green-900 bg-green-50 rounded-lg text-[12px]' 
                    placeholder='Some more words, maybe?'
                    value={formDataCareerMessage.message}
                    onChange={(e) =>
                      setFormData((prev) => ({ ...prev, message: e.target.value }))
                    }></textarea>
                <button onClick={() => (handleCareerMessage())} className='text-[12px] flex justify-center items-center lg:hidden font-bold text-white w-full border py-3 rounded-lg bg-[#007924] hover:bg-[#44975d]'>
                    {isLoading ? "Try Send..." : "Send"}
                </button>
            </div>
        </>
