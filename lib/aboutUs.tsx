...
import { AboutImage, AboutText } from './adminAboutUs';
...
function AboutUsComponent(props: { id: string } & object, ref: React.Ref<HTMLDivElement>) {
  ...
  //   'about-1.jpeg',
  // ];
  const jumlahGambar = aboutImage.length;
  const [selectImg, setSelectImg] = React.useState(0);
  const handleClick = () => {
    if (selectImg === jumlahGambar - 1) {
      setSelectImg(0);
    } else {
  ...
  return (
    ...
    <div className='lg:mt-100 lg:flex lg:flex-row lg:justify-center lg:items-center lg:w-full lg:relative flex flex-col justify-center items-center relative w-full gap-y-10'>
      <div className='lg:w-270 lg:pl-25 w-70'>
        ...
        {valuesShow.length > 0 && valuesShow[0].values ? (
          valuesShow[0].values.map((val: string, index: number) => (
            <p 
              key={index}
              className="lg:w-130 lg:mb-5 lg:text-justify lg:text-lg w-70 mb-5 text-justify text-xs"
            >
              {val}
            </p>
          ))
        ) : (
          <p className="tlg:w-130 lg:mb-5 lg:text-justify lg:text-lg w-70 mb-5 text-justify text-xs">Loading...</p>
        )}
      ...
      {aboutImage.length > 0 ?
        <div className='relative'>
          <Image width={140} height={140} src={`/uploads/aboutUs/${aboutImage[selectImg].fileName}`} alt="golang" className=' lg:w-[800px] lg:h-[360px] lg:rounded-xl w-[300px] rounded-xl object-cover' />
          <button onClick={handleClick}>
            <Image width={140} height={140} src={`/uploads/aboutUs/${selectImg === jumlahGambar-1 ? aboutImage[selectImg - (jumlahGambar-1)].fileName : aboutImage[selectImg+1].fileName}`} alt="golang" className=' lg:w-[170px] lg:rounded-xl lg:absolute lg:z-2 lg:-bottom-10 lg:-right-10 lg:hover:w-[180px] lg:hover:-right-5 lg:transition-all lg:duration-200 lg:hover:shadow-lg w-[100px] rounded-xl absolute z-2 -bottom-10 -right-5 hover:w-[180px] hover:-right-5 transition-all duration-200 hover:shadow-lg object-cover animate-fade-in' />
          </button>
          <Image width={140} height={140} src={`/uploads/aboutUs/${
            selectImg === jumlahGambar-2 ? aboutImage[selectImg - (jumlahGambar-2)].fileName 
            : selectImg === jumlahGambar-1 ? aboutImage[selectImg - (jumlahGambar-1)].fileName
            : aboutImage[selectImg+2].fileName}`} alt="golang" className=' lg:w-[200px] lg:rounded-xl lg:absolute lg:z-2 lg:bottom-30 lg:-right-70 w-[150px] rounded-xl absolute z-2 bottom-20 -right-50 object-cover' />
