...
export default function AdminNavbar(props: AdminNavbarProps) {
  ...
  return (
    <div className='fixed top-0 w-50 h-158 flex flex-col justify-start items-center pt-8 border-r-[3px] border-[#9e1ac3] bg-[#ffffff]'>
        ...
        <button className='w-full px-7 py-2 flex flex-row items-center gap-2 hover:bg-[#f1e7ff]'>
        ...
        <button className='w-full px-7 py-2 flex flex-row items-center gap-2 hover:bg-[#f1e7ff]'>
            <Image width={30} height={30} src={navActive === 'user' ? `/user-active.svg` : '/user.svg'} alt="Dashboard" className='w-3 h-3'/>
            <p className={`text-[12px] ${navActive === 'user' ? 'text-[#AD48FF] font-bold' : 'text-black'}`}>Users</p>
        </button>
        {(navActive === 'user' || navActive === 'customer' || navActive === 'admin') &&
          <div className={`w-full mb-2 flex flex-col bg-[#fbeffe]`}>
            <button className='w-full pl-11 py-2 flex flex-row items-center gap-2 hover:bg-[#f1e7ff]'>
        ...
        <button className='w-full px-7 py-2 flex flex-row items-center gap-2 mb-2 hover:bg-[#f1e7ff]'>
